...
suite("PoParser", () => {
  ...
  test("reports invalid msgstr format (missing quote) as error", async () => {
    ...
    const res = await parser.parse(uri, content);
    assert.strictEqual(res.success, false);
    assert.ok(res.diagnostics.some(d=>/invalid msgstr format/.test(d.message) && d.severity === MyDiagnosticSeverity.Error));
  });
  ...
  test("reports missing msgstr as error", async () => {
    ...
    const res = await parser.parse(uri, content);
    assert.strictEqual(res.success, false);
    assert.ok(res.diagnostics.some(d=>/missing msgstr/.test(d.message) && d.severity === MyDiagnosticSeverity.Error));
  });
  ...
  test("reports invalid msgid format (missing quote) as error", async () => {
    ...
    const res = await parser.parse(uri, content);
    assert.strictEqual(res.success, false);
    assert.ok(res.diagnostics.some(d=>/invalid msgid format/.test(d.message) && d.severity === MyDiagnosticSeverity.Error));
  });
  ...
  test("reports unrecognized line as warning", async () => {
    ...
    const res = await parser.parse(uri, content);
    assert.strictEqual(res.success, false);
    assert.ok(res.diagnostics.some(d=>/unrecognized line in \.po/.test(d.message) && d.severity === MyDiagnosticSeverity.Warning));
  });
});
